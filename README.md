...
| ☁️ Backend / BaaS | Supabase (PostgreSQL + Auth + Storage) |
...
**🗂️ Patrón repositorio:** toda la lógica de acceso a datos se abstrae detrás de interfaces en `repositories/`. Los componentes y stores solo conocen la interfaz, no la implementación de Supabase.
...
**⚠️ Prevención de race conditions:** las funciones de carga críticas usan un ID de petición (`requestId`) para descartar respuestas de peticiones obsoletas, especialmente relevante en React StrictMode.
...
| 🛡️ `admin` | Acceso completo: gestión de usuarios, servicios y estadísticas globales |
